...
const EventCategories = ({ events, searchTerm }: { events: Event[], searchTerm: string }) => {
    ...
    const indiaEvents = filteredEvents.filter(event => 
        event.location.toLowerCase().includes('india')
    );
    
    const blueChipEvents = filteredEvents.filter(event => 
        !event.location.toLowerCase().includes('india')
    );
                        ...
                        <motion.div className="w-full py-10 max-w-[280px] sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto px-4 mb-8">
                            <div className="relative group">
                                ...
                                <div className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2">
                                    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400 group-hover:text-white/70 transition-colors duration-300" fill="none" strokeWidth="2" stroke="currentColor" viewBox="0 0 24 24">
